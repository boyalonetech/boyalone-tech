...
const TestimonialCard = () => {
  ...
  return (
    <div className="py-16 px-6 ">
      <h2 className="3xl font-bold center mb-12" id="review">
        What Clients Say
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-7xl mx-auto">
        {testimonials.map((t, index) => (
          <div
            key={index}
            className=" w-full h-[350px] p-6 rounded-xl shadow hover:shadow-lg transition flex flex-col items-center center"
          >
            {/* Avatar */}
            ...
            {/* Name & Role */}
            <h4 className="font-semibold lg">{t.name}</h4>
            <p className="sm gray-500 mb-2">
              {t.role} at {t.company}
            </p>

            {/* Message */}
            <p className="gray-700 mb-3 sm leading-snug line-clamp-3">
              {t.message}
            </p>

            {/* Star Rating */}
            <div className="flex justify-center yellow-400">
              {[...Array(5)].map((_, i) =>
                i < t.rating ? (
